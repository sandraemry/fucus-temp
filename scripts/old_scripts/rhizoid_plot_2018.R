## 2018 Bamfield, Nanaimo and Ruckle Park at different temperatures

library(tidyverse)
library(RColorBrewer)

rhiz <- read_csv("./data/tpc_rhizoid_tidy_2018.csv", col_types = cols(
  site = col_factor(),
  temp = col_double(),
  rep = col_double(),
  photo_id = col_double(),
...
rhiz_sum <- rhiz %>%
  ...
  summarise(avg_length = mean(length), 
            se_length = sd(length)/sqrt(10))

# need to add error bars in
ggplot(data = rhiz_sum, aes(x = temp, y = avg_length, color = site)) + 
  geom_point() + 
